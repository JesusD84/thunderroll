...
def downgrade() -> None:
    ...
    sa.Column('status', postgresql.ENUM('AVAILABLE', 'SOLD', 'IN_TRANSIT', 'RESERVED', 'DAMAGED', 'MAINTENANCE', name='unitstatus'), autoincrement=False, nullable=False),
    ...
    sa.ForeignKeyConstraint(['model_id'], ['models.id'], name='units_model_id_fkey'),
    ...
    sa.Column('engine_type', sa.VARCHAR(length=50), autoincrement=False, nullable=True),
    sa.Column('displacement', sa.VARCHAR(length=20), autoincrement=False, nullable=True),
    sa.Column('is_active', sa.BOOLEAN(), autoincrement=False, nullable=True),
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('now()'), autoincrement=False, nullable=True),
    sa.ForeignKeyConstraint(['brand_id'], ['brands.id'], name='models_brand_id_fkey'),
